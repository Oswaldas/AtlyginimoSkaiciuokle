...
namespace Atlyginimo_skaičiuoklė
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void GetPercentageTBox(out double IncomePercent, out double HealthPercent, out double PensionPercent, out double EmployeePercent, out double AuthorsPercent, out double ClientPercent)
        {
            //Get Nulls If Textboxes Are Empty
            IncomePercent = HealthPercent = PensionPercent = EmployeePercent = AuthorsPercent = ClientPercent = 0;
            double.TryParse(tBoxPajamuProc.Text, out IncomePercent);
        ...
        private void butSkaiciuotiAtlyginimaIRankas_Click(object sender, EventArgs e)
        {
            ...
            double IncomePercent, HealthPercent, PensionPercent, EmployeePercent, AuthorsPercent, ClientPercent;
            GetPercentageTBox(out IncomePercent, out HealthPercent, out PensionPercent, out EmployeePercent, out AuthorsPercent, out ClientPercent);
            ...
            IncomeTax = Math.Round((SalaryOnPaper * (IncomePercent / 100)), 2);
            ...
            lblPajamuProc1.Text = (IncomePercent / 100).ToString("P");
        ...
        private void buttonSkaiciuotiIRankas_Click(object sender, EventArgs e)
        {
            ...
            {
                double AuthorsSalaryOnPaper = Math.Round(AuthorsSalaryInHand * (100 / (100 - (IncomePercent + HealthPercent + PensionPercent))), 2);
                double AuthorsIncome = Math.Round((AuthorsSalaryOnPaper * (IncomePercent / 100)), 2);
                ...
                double SalaryOnlyOnPaper = Math.Round(SalaryInHand * (100 / (100 - (IncomePercent + HealthPercent + PensionPercent))), 2);
                double IncomeTax2 = Math.Round((SalaryOnlyOnPaper * (IncomePercent / 100)), 2);
            ...
            {
                SalaryOnPaper = Math.Round(AuthorsSalaryInHand * (100 / (100 - (IncomePercent + HealthPercent + PensionPercent))), 2);
                IncomeTax = Math.Round((SalaryOnPaper * (IncomePercent / 100)), 2);
                SodraHealthInsurance = Math.Round((SalaryOnPaper * (HealthPercent / 100)), 2);
                SodraPensionInsurance = Math.Round((SalaryOnPaper * (PensionPercent / 100)), 2);
            }
            ...
            {
                SalaryOnPaper = Math.Round(SalaryInHand * (100 / (100 - (IncomePercent + HealthPercent + PensionPercent))), 2);
                IncomeTax = Math.Round((SalaryOnPaper * (IncomePercent / 100)), 2);
                SodraHealthInsurance = Math.Round((SalaryOnPaper * (HealthPercent / 100)), 2);
                SodraPensionInsurance = Math.Round((SalaryOnPaper * (PensionPercent / 100)), 2);
                EmployersFees = Math.Round((SalaryOnPaper * (EmployeePercent / 100)), 2);
            ...
            lblPajamuProc.Text = (IncomePercent / 100).ToString("P");
